...
/**
 * Class block_kineo_user_profile_edit_form
 *
 * This is the edit form for the block
 */
class block_kineo_user_profile_edit_form extends block_edit_form {
    ...
    protected function specific_definition($mform) {
        ...
        if (has_any_capability(['moodle/block:edit', 'moodle/user:manageblocks'], $this->block->context)) {
            ...
            $admintree = block_kineo_user_profile::block_user_profile_settings($tree);
            ...
            foreach ($categories as $category => $cat) {
                ...
                if (sizeof($nodeobjs) > 1) {
                    $groups[$category] = $mform->createElement(
                        'group',
                        $category . '_grp',
                        get_string('hidecategorynode', 'block_kineo_user_profile', $cat->title),
                        $nodeobjs,
                        '</br>',
                        false
                    );
            ...
            $mform->addElement('select', 'config_category', get_string('displaycategory', 'block_kineo_user_profile'), $catitems);
            ...
            $mform->addElement('static', 'warning', '', get_string('userwarning', 'block_kineo_user_profile'));
                  return;
        }
    }
}
