...
use block_kineo_user_profile\helper;

require_once $CFG->dirroot . '/blocks/kineo_user_profile/classes/helper.php';
 ...
 * Main block file
 */
class block_kineo_user_profile extends block_base {

    /**
     * Initialises this block instance
     */
    public function init() {
        $this->title = get_string('pluginname', 'block_kineo_user_profile');
    }
    ...
    private function tree_filter($tree) {
        ...
        $tenant_profilefield_tree = helper::get_tenant_profile_field_tree($USER->tenantid);
        ...
        foreach ($tree->categories as $category => $cat) {
            if ($configcategory == $category) {
                ...
                if (strpos($cat->classes, $cat->name) === false) {
                    $catclasses =  trim('block_kineo_user_profile_category_' . $cat->name . ' ' . $catclasses);
                }
                ...
                foreach ($cat->nodes as $nodename => $node) {
                    // VTNHAS-504
                    if(strpos($nodename, 'custom_field_') !== false && empty($tenant_profilefield_tree[$nodename])) {
                        continue;
                    }
